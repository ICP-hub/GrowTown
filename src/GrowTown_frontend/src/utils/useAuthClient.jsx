...
export const useAuthClient = () => {
  ...
  const backend_id = process.env.CANISTER_ID_GrowTown_BACKEND;
  const frontend_id = process.env.CANISTER_ID_GrowTown_FRONTEND;
  ...
  async function plugConnectMobile() {
    try {
      ...
      if (isMobile) {
        ...
        const actor = await createActor(process.env.CANISTER_ID_GrowTown_BACKEND, {
          agent,
        });
        ...
        //   canisterId: process.env.CANISTER_ID_GrowTown_BACKEND,
        ...
        if (
          ...
          if (provider === "plug") {
            ...
            const backendActor = createActor(
              process.env.CANISTER_ID_GrowTown_BACKEND,
  ...
  const updateClient = async (client) => {
    try {
      ...
      const backendActor = createActor(process.env.CANISTER_ID_GrowTown_BACKEND, {
        agentOptions: { identity, verifyQuerySignatures: false },
      });
      const ledgerActor1 = createLedgerActor(ledgerCanId, { agent });
      setLedgerActor(ledgerActor1);
