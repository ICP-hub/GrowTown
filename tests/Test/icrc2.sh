...
export MINTER_ACCOUNT_ID=$(dfx ledger account-id --playground)
echo "MINTER_ACCOUNT_ID: $MINTER_ACCOUNT_ID"

# Step 3: Switch back to the default identity
dfx identity use Mohit
export DEFAULT_ACCOUNT_ID=$(dfx ledger account-id --playground)
...
dfx deploy icrc2_token_canister --playground --argument "
  (variant {
    Init = record {
...
dfx canister call icrc2_token_canister --playground name
echo "ICRC2 token canister deployed and token name retrieved"

# Step 7: Check balance of the default account
balance=$(dfx canister call icrc2_token_canister --playground account_balance_dfx "(record {account = \"$DEFAULT_ACCOUNT_ID\";})")
